...
kafka_client_test_() ->
    ...
    , fun (ClientSup) ->
	      ...
	      , ?_assertMatch({ok, _}, kafka_cluster_client:fetch(ClientSup, ?CLIENT_ID, <<"test">>, 0, 0))
...
broker_connection_test_() ->
    ...
    [?_assertMatch({ok, #metadata_response{}}, broker_connection:metadata(Pid, ?CLIENT_ID, #metadata_request{ topics = [<<"test">>] })),
